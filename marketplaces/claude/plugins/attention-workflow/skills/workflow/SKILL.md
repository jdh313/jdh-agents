...
name: workflow
description: >-
  Run a code change through the attention-regulating lifecycle — Frame, Design,
  Prepare, authorize, Implement, Verify, Deliver, Close — with versioned
  authority, local run state, and an independent verifier whose verdict stays
  withheld until you have committed your own judgment. Use when starting,
  resuming, authorizing, verifying, or closing a change under this workflow,
  when the SessionStart card shows an active change, or when the user says
  "frame this change", "authorize", "candidate ready", "reconcile", "where does
  this change stand", "supersede the grant", or "close this out". Experimental;
  supersedes spec-flow while enabled.
