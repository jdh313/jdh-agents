...
6. **Marker label** *(optional, at most one)* — currently only `chart:map`. Ungrouped, and additive to the surface + type pair rather than a substitute for either. See the `linear` skill for the rule on minting new markers.
