...
description: >-
  Review an adapted/borrowed skill against its upstream source — compare
  behavior (not just text), flag dropped moves, silent divergences, and
  fabricated attributions, then refresh the pinned upstream commit SHA. Use when
  adopting a skill from another repo, when an upstream skill may have changed,
  or when the user says "review this against upstream", "check upstream drift",
  "did upstream change", "is our adaptation still honest".
allowed-tools:
  - Bash(gh api *)
  - Bash(base64 *)
  - Read
