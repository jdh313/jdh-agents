...
name: vault-inspect
description: >-
  Run a diagnostic sweep on the Obsidian vault. Reports structural issues
  (orphans, dead-ends, broken links, frontmatter, stale pages) and/or
  wiki-semantic issues (page_type validation, neutral-definition opening,
  source-link format, Software Catalog schema, anti-staleness). Use when user
  says "lint the wiki", "wiki health check", "check knowledge base", "vault
  health check", "note health", or invokes `/vault-inspect`. Merger of the
  former `note-health` and `wiki-lint` skills.
