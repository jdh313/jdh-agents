...
topic: {Topic}
started: YYYY-MM-DD
up: "[[{Existing parent MOC}]]"   # OPTIONAL — include only after verifying the note exists. Omit otherwise.
...
- **Link to a real parent or omit `up`.** Verify that the parent MOC note exists before setting `up`. If the context has no real parent MOC, omit the field; never synthesize a wikilink from the folder name or create a MOC merely to fill the template.
- **Do not duplicate folder context in tags.** The workspace path carries its primary context. Add a `context/{y}` tag only when the live vault guidance calls for a genuinely cross-cutting context that the folder does not already convey.
