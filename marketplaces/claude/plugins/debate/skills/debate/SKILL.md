...
name: debate
description: >-
  This skill should be used when the user asks for an opinion on a decision, or
  when genuine decision/opinion questions are detected ("should I", "is it
  worth", "would you recommend", "X vs Y"). Spawns parallel advocate agents to
  research opposing perspectives, with optional fact-checking, devil's advocacy,
  and independent synthesis. Three modes (Quick/Standard/Deep) control pipeline
  depth. Not for factual questions, simple preferences, or single-answer
  lookups.
allowed-tools:
