...
description: >-
  Generate a systems-first parallel-research handoff prompt from a locked needs
  map or requirements doc. This skill should be used when a needs map exists
  (typically produced by `first-principles`) and the user wants to research the
  solution space — "research options against the needs map", "generate the
  research prompt", "find systems that solve this", "send off subagents to
  research X". Produces a prompt for a fresh session that dispatches parallel
  research subagents — each designing one end-to-end system (backbone + tools
  assigned to roles) — with real-incident test scenarios as gates,
  primary-source citation requirements, an adversarial verification pass
  attacking the seams between tools, and composition-aware synthesis. Not for
  running research directly in-session (use `deep-research` for that).
---
