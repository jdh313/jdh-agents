...
name: lead
description: >-
  Engineering-manager mode: the user names the work (a ticket, several tickets,
  a milestone), and this skill decides how it gets executed, dispatches
  subagents, verifies their output by diff, and stops before anything
  outward-facing. No fixed lifecycle: a ticket that is already shaped goes
  straight to dispatch; one that is not is scoped inline first. Works with
  Linear, Fibery, and GitHub Issues. This skill should be used when the user
  invokes `/em:lead`, says "be my engineering manager", "run this ticket", "take
  TEAM-123 from here", "orchestrate this", "work the milestone", or hands over
  shaped tickets for execution. For a brief only, with no execution, use
  `em:start`.
