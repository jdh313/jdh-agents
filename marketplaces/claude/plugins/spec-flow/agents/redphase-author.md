---
name: redphase-author
description: Compiles a spec-flow contract's *Done when* bullets into automated tests that FAIL, commits them as a checkpoint before any implementation code exists, verifies that checkpoint is isolated from the working copy, and returns a per-bullet coverage manifest (never the test source). Author-is-not-implementer by construction — it is given the contract, not the plan. Dispatched by `spec-flow:implement` (step 4.5), between cadence negotiation and execute.
model: sonnet
color: red
tools:
  - Bash
