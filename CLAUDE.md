...
│   └── marketplace/              # CLI tool: sync, validate, lint, scan, check
└── README.md
```
...
`marketplace scan`, run by the prek pre-push hook and by `marketplace check`.
...
| `scripts/marketplace/` | AgentForge orchestration, validation/lint, and the privacy gate | Single entrypoint for all automation (`uv run marketplace <command>`) |
