...
3. Run `uv run marketplace validate` → validates marketplace.json structure
...
3. Runs `uv run marketplace check` (sync-drift + schema + lint)
...
| `scripts/marketplace/` | `marketplace` CLI: discovers plugins, validates/lints, syncs both manifests, exports the public subset | Single entrypoint for all automation (`uv run marketplace <command>`) |
...
- **Plugin discovery:** `marketplace sync` (the `discovery` module) uses `rglob("plugin.json")` to find all plugins recursively. Ensure plugin metadata is always in `.claude-plugin/plugin.json` relative to the plugin directory root.
