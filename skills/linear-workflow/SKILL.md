...
- **spec-flow** — When a contract is hosted in Linear (e.g. `/spec-flow start TEAM-123`), spec-flow writes the contract body to the ticket description using the full five-section template. This skill governs the surrounding ticket fields. spec-flow does not set labels, priority, or milestone — those follow the conventions here.
