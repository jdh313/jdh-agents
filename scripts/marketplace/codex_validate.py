"""Validate jdh-agents's generated local Codex publication layout.

Codex does not currently expose a non-interactive ``plugin validate`` command,
so jdh-agents validates the generated local publication shape it owns. This
...
def validate_codex_marketplace(manifest: Any, plugins_root: Path) -> list[str]:
    """Return errors for jdh-agents's materialized local Codex publication.
