...
description: Re-evaluate an existing Software Catalog entry against its own revisit triggers — read when it was last evaluated, research what changed in the tool since then, score the changes against each trigger, and record the outcome. Use when the user says "recheck this catalog entry", "re-evaluate X", "is X still on hold", "did anything change with X since we last looked", "has X's revisit trigger fired", or "catalog recheck". Discovers whether a stance SHOULD change; hands the actual write to `catalog-evaluate`.
allowed-tools:
  - Read
  - Glob
  - Grep
