...
/obsidian-curator:cleanup --type=stale
/obsidian-curator:cleanup --type=frontmatter
/obsidian-curator:cleanup --type=conventions
```
...
---
Run `/obsidian-curator:vault-health` to see the full report.
```
...
- Run `/obsidian-curator:cleanup` again to continue
- Or run `/obsidian-curator:cleanup --type=X` to focus on remaining issues
...
This command launches the `vault-curator` agent:

```python
# The cleanup command invokes the vault-curator agent
# with appropriate context based on arguments

if args.type:
    # Focus on specific category
    context = f"Focus on {args.type} issues only"
elif args.folder:
    # Limit to folder
    context = f"Only check notes in {args.folder}"
else:
    # Full interactive session
    context = "Start interactive cleanup, let user choose category"

# Launch vault-curator agent
Task(
    subagent_type="vault-curator",
    prompt=f"Start a vault cleanup session. {context}"
)
```

## Related Commands

- `/obsidian-curator:vault-health` — Quick audit without interactive session
