...
description: >-
  ...
  level, and land it as a durable vault note that later studies add to (plus
  an interactive Artifact page where the Artifact tool exists). Use only when
  the user names an outside repo or project and wants its internals —
  "teardown X", "study home-assistant/core", "how is <outside project>
  architected under the hood", "walk me through <repo>'s source", "how does
  <repo> implement <mechanism>", "add <repo> to that survey". Not for the
  user's own repos, general vault or wiki questions, or a quick library API
