...
| added | "A repo earns a CONTEXT.md" worthiness gate, plus composition notes with `spec-flow:start`, `/capture-decision`, `/drift-check` | Ecosystem integration; avoids spawning glossaries for repos already covered by external sources (vault wiki / Work docs). |
