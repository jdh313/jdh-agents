...
**Proprietary repos:** for work/Carta repos, the personal vault still holds your personal understanding records, but durable _shared_ facts must **not** leak into the vault — graduate them to the repo's own `CONTEXT.md` or an internal authority, exactly as `grill-with-docs` does.
