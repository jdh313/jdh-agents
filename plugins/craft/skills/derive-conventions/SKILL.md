...
description: Derives a host repository's unwritten conventions — naming, error-handling shape, docstring format, CLI voice — from its own code, presents each inferred rule with its evidence for accept/edit/reject, and writes the approved set to CONVENTIONS.md so craft's review agents stop re-deriving them every run. Never writes an unreviewed rule; the human adjudicates every entry.
when_to_use: Use when the user says "derive the conventions", "set up CONVENTIONS.md", "capture this repo's house style", "stop re-deriving the conventions", "refresh the conventions file", or after the first run of house-style-reviewer / comment-reviewer / copy-reviewer surfaces rules worth pinning.
argument-hint: "[lane: style | comments | copy | all, or a path to scope to]"
effort: high
allowed-tools:
  - Read
  - Grep
