...
5. **Implementation** — At handoff, AI asks *"all at once, or check in after a piece?"* and may propose a sensible breakpoint. Cadence decided per session, not persisted. Then a **red phase**: the **`redphase-author`** agent compiles each *Done when* bullet into an automated test that **fails**, and commits those tests before any implementation code exists. In jj, it must leave a fresh empty working-copy revision whose parent is that test checkpoint; describing `@` alone does not create the boundary. The implement orchestrator checks this VCS evidence before implementation begins. The author is deliberately isolated — it gets the contract, never the plan — because a test author who can see the implementation writes tests the implementation passes. Implementation then works those tests green. Before declaring done, the **`contract-verifier`** agent independently runs each *Done when* bullet in isolated context and returns a met / not-met / drifted verdict — verification is behavioral, not a diff-read. Two gates, different evidence.
