...
3. **Drafting** — The contract is a **worksheet** ordered by audience: cold-legible *front-matter* (*What we're doing*, *Why*, *Out of scope*, *Done when*) sits above an internal *working-matter* ledger (*Approach / wiring* plus a three-state *Decision log* whose rows are born `[open]`, resolve to `[resolved]`, or punt to `[deferred]`). Bullets/lists/tables, never prose. See `references/contract-template.md`.
