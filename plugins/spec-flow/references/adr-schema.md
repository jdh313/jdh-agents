...
The forces that bound the decision space. Technical (must run on Python 3.14), regulatory (must keep PHI out), operational (no team to self-host), business (Freddie's two optimization axes), or stylistic (must compose with existing CLAUDE.md gotchas).
