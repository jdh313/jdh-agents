...
- **`linear`** — a Linear issue (e.g. `TEAM-49`), whose description holds the contract body.
...
|---|---|---|
| Bare ticket token (`TEAM-49`) | linear | The ID alone treated as the contract |
| `the contract is TEAM-49` / `use TEAM-49 as the contract` | linear | Explicit framing |
| `implement TEAM-49` / `pick up TEAM-49` | linear | Action-on-ticket = ticket-is-contract |
| `see TEAM-49` / `regarding TEAM-49` / `the work in TEAM-49` | ask once | Ticket present but framed as reference — ambiguous |
...
The ambiguous case escapes with a one-line ask: *"Use TEAM-49 itself as the contract, or draft a `.docs/` file that references TEAM-49?"* — do not silently pick.
...
- Surface the fact: *"Linear MCP server isn't connected — I can't read or write TEAM-49."*
