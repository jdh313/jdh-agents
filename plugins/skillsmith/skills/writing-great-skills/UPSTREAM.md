...
| added | One `ADDENDA.md` context pointer near the top of `SKILL.md`, and the `ADDENDA.md` file itself. | Bridges the generic reference to jdh-agents's own authoring mechanics (provenance, `allowed-tools` vs agent `tools:`, auto-discovery layout, commit format) that upstream doesn't cover. Pointer wording fires only when authoring *in this repo*, so it costs nothing on other reads. |
