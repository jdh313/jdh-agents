...
Two load-bearing checks (failure here means `fail`), then a battery of mechanical checks (failure here means `fail` with `severity: mechanical` — `persist.py` will catch them too, but flagging early saves a round trip).
...
### Mechanical checks (soft gate — `persist.py` enforces hard)

For each, emit a `severity: mechanical` issue. The orchestrator can choose to surface to user or fix and re-call.

- **Required frontmatter fields present and non-null:** `id` (may be `"PLACEHOLDER"` in pre-persist mode), `title`, `status`, `decision_date`, `project`, `area`, `topic`, `reversibility`. `supersedes:` must be **present** (may be `[]`).
...
- **`area:` and `topic:`** are non-null and (in audit mode) present in the on-disk taxonomy YAML. In pre-persist mode, only check that the values are non-null — taxonomy validation lives in `persist.py`.
- **`tags:`** contains `decision`.
- **`aliases:` slugs** start with `ndr-`. Pre-persist mode does NOT check vault-wide uniqueness (that's `persist.py`'s job and needs disk access).
