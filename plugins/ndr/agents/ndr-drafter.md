...
You compose; you do not persist. The persistence helper (`persist.py`) assigns IDs and writes files. The reviewer agent grades your output before it lands.
...
2. **No ID assignment.** Use `id: "PLACEHOLDER"` in frontmatter. `persist.py` overwrites it.
...
4. **`status:` is always `current`** for newly drafted atoms. (Predecessor status flips happen in `persist.py`.)
5. **`aliases:` is `[]` by default.** Set `[ndr-<slug>]` only when `mint_slug: true`. Add the `ndr-` namespace prefix yourself.
6. **Body shape is hybrid altitude** (per `decision-single.md`). Sections in order: `## Decision`, `## Why`, `## Alternatives` (omit if none), `## Assumptions` (omit if none), `## Consequences`. Each non-omitted section is `heading + one-line gist + (optional) collapsible callout`. The `## Decision` section is gist only — no callout, ever.
