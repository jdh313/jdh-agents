...
description: >
  Read-only drift-check and classification agent for adapted skills.
  Compares upstream skill bytes to a local adaptation, classifies each
  behavioral unit (kept / diverged / dropped / added), and flags
  fabricated attributions. Dispatch for multi-skill batch reviews so
  comparison state stays isolated per skill and the orchestrating skill
  can work interactively. Never writes — surfaces findings only.
tools:
  - Read
  - Grep
  - Glob
