...
2. **Project** — assigned to the active CartaOS phase project (rotates per phase)
...
Example:
> Carta Workspace's API Access controls block gcloud's default OAuth client from requesting Workspace scopes. Need a custom Desktop OAuth client in a Carta-owned GCP project so `cartaos sheets fetch` works from local dev.
...
- **No Linear issue IDs inside source code, comments, docstrings, ADRs, or NDRs.** Per CartaOS `CLAUDE.local.md`: references go ticket → code, not code → ticket.
...
- **CartaOS `CLAUDE.md`** — repo-level conventions and known gotchas
- **`Linear Setup.md`** in the vault — workspace shape (cycles, milestones, label groups, projects)
- **`CLAUDE.local.md`** in the CartaOS repo — per-user conventions (e.g. no Linear IDs in code)
