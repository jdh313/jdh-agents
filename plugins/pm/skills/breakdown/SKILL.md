...
description: >-
  ...
  set of smaller actionable tickets. Grounds against current NDR heads
  ...
  blocks/blocked-by relations, and optionally recommends a spec-flow
  contract for slices large enough to merit one. Slices land in Backlog
  with default priority — cycle assignment is `pm:groom`'s job.
argument-hint: "[source — CAR-N ticket, vault note path, ndr:atom-id, file path; falls back to conversation]"
allowed-tools:
  ...
  - Glob
  - Bash
  # Compose with ndr:ground, linear-workflow, spec-flow:start
  - Skill
  - Agent
---
...
Decompose a goal / plan / spec / vault note into independently-grabbable Linear tickets using **vertical slices** — tracer bullets that cut through every layer end-to-end, not horizontal slices of one layer. Grounds the breakdown against current NDR heads first so slices don't conflict with settled architecture. Publishes in dependency order via the linear plugin with native Linear blocks/blocked-by relations.

This is the multi-ticket-from-a-plan skill. For one-shot single-ticket drafting, use `pm:author` (when it ships).

## Inputs

- **Source (optional argument):** A `CAR-N` ticket (parent), vault note path, `ndr:<atom-id>`, or file path. Falls back to conversation context.
- **Linear team:** `CAR` (Carta Healthcare).
...
- **Codebase root:** the current CartaOS repo (`/Users/carta/projects/cartaos`) — for optional exploration when slices touch tracked areas.

## Procedure

1. **Gather context.** Read the conversation. If an argument was passed, read the source fully:
   - `CAR-N` — fetch via `mcp__linear-server__get_issue`. Record as the **parent ticket** for later linking.
...
2. **Ground against NDRs.** Dispatch `Skill(ndr:ground)` against the goal's area (auth, backend framework, frontend, infra, etc.). Surface the current decision heads relevant to the breakdown. Use them to:
   - Avoid proposing slices that conflict with active NDRs
...
6. **Confirm parent linking.** If the source was a `CAR-N` ticket, each child will get a Linear `parent` relation pointing at it. Confirm the parent ID once before publishing. If the source was vault/NDR/conversation, no Linear parent — each child's `## Context` describes the goal directly (link to vault note or NDR if applicable).

7. **Publish in dependency order via `linear-workflow`.** Save blockers first so children can reference real `CAR-N` IDs in the Linear blocks/blocked-by relation. For each slice:
   ...
   - Set project: active CartaOS phase project (lookup via `mcp__linear-server__list_projects`, take the non-completed one).
   - Set parent relation if applicable.
   - After save, capture the returned `CAR-N` for downstream blocks/blocked-by references.

8. **Flag spec-flow candidates.** After publishing, scan the published slices for ones that warrant a spec-flow contract — multi-day work, architectural negotiation needed, or contract-shaped (clear handoff between user and AI). For each candidate, recommend `Skill(spec-flow:start)` with the slice's CAR-N as the contract host.
...
- **`ndr:ground`** (ndr plugin) — grounding pass before slicing. Surfaces current decision heads relevant to the goal's area.
- **`ndr:decisions`** (ndr plugin) — used when the source argument is an `ndr:` reference.
- **`ndr:capture-decision`** (ndr plugin) — recommended after a `Decision`-type slice's call is made.
...
- **`spec-flow:start`** (spec-flow plugin) — optional handoff for slices that warrant a contract-tracked workflow.
...
- **CartaOS `CLAUDE.md`** — repo conventions; the codebase shape that informs which layers a vertical slice touches.
