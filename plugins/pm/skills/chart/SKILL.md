...
description: >-
  ...
  This skill should be used when the user invokes `/pm:chart`, says "chart
...
allowed-tools:
  # Linear — read-side only; the map, its tickets, relations, and resolution
  # comments are all written by the `linear-ops` agent
  ...
  - Glob
  - Bash(ndr *)
  # Compose with craft:grill, craft:domain-modeling, craft:prototype, ndr;
  # publish via `linear-ops`
  - Skill
  - Agent
upstream:
