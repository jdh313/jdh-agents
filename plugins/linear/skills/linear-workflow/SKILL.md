...
description: This skill should be used when creating, updating, transitioning, reading, or querying Linear tickets in the AcmeOS workspace. Trigger phrases include "open a linear ticket", "create a CAR ticket", "add to linear", "add to backlog", "log this in linear", "move TEAM-N to", "transition TEAM-N", "promote to todo", "back to backlog", "mark TEAM-N done", "what state is TEAM-N", "set TEAM-N priority", "what's in todo", "show my linear tickets". Supplies ticket creation defaults (team, labels, priority, milestone), status flow semantics, title and description conventions, and MCP call patterns. Does NOT cover the spec-flow contract lifecycle (use spec-flow plugin) or the decision of whether to open a ticket at all (project CLAUDE.md owns the floor rule).
---

# linear-workflow

Operational conventions for AcmeOS Linear work. Loads on any ticket create, transition, read, or update.
...
- **Currently scoped to:** the `CAR` (Work Healthcare) team. Linear is AcmeOS-only — scope decision lives in `~/Loose Ends/Reference/Tools/Software Catalog/Linear.md`.

## Conventions

### Team and project

- **Team:** Always `CAR` (Work Healthcare). One team only.
- **Project:** Look up the active project via `mcp__linear-server__list_projects` and pick the non-completed AcmeOS project. Projects are phase-scoped (one per 60-day phase) and rotate — don't hardcode the current name.
...
- **Backlog → Todo is an authorization step.** Approval comes from Stevie (stakeholder) OR self-confirmation that the work fits an existing milestone / phase scope.
...
mcp__linear-server__save_issue(
    team="CAR",
    project="<active AcmeOS project>",       # look up via list_projects; phase-scoped, rotates
...
1. Confirm with the user that the ticket has been approved (Stevie nod OR clear scope justification). Don't promote silently.
...
- **spec-flow** — When a contract is hosted in Linear (e.g. `/spec-flow start TEAM-49`), spec-flow writes the contract body to the ticket description using the full five-section template. This skill governs the surrounding ticket fields. spec-flow does not set labels, priority, or milestone — those follow the conventions here.
...
- **Project CLAUDE.md (AcmeOS)** — Owns the floor rule: "can't do now, or has dependencies to sequence" → open a ticket. This skill takes over once the decision to open has been made.
...
| Stakeholder views (`stakeholder:*` labels, Stevie-specific view) | Declined per `Linear Setup — Views.md` | Only if/when stakeholder gets direct Linear access |
