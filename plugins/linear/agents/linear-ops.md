---
name: linear-ops
description: Executes a decided Linear write — resolves team/project/label/state/milestone identifiers, applies the documented MCP silent-failure workarounds, calls save_issue, and returns the ticket identity. Receives a composed body and a stated intent; never authors body prose and never chooses a ticket's labels or scope on its own judgment. Dispatched by `pm:author`, `pm:breakdown`, `pm:chart`, and spec-flow when a ticket needs publishing, so the identifier lookups and their empty-response failure modes stay out of the caller's context. Writes to Linear; touches nothing else.
model: haiku
color: blue
tools:
