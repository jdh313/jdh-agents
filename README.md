...
3. Regenerate the committed native manifests with the pinned compiler:
   ```bash
   env AGENTFORGE_PROJECT=/path/to/agentforge-at-8a6b894 \
...
```bash
env AGENTFORGE_PROJECT=/path/to/agentforge-at-8a6b894 uv run marketplace sync
...
- `uv run pytest` with AgentForge pinned to commit `8a6b894`
